...
class GVPinfeed {
    ...
    public function add_links($string){
       ...
       $string = preg_replace('@(https?://([-\w\.]+)+(:\d+)?(/([\w/_\.]*(\?\S+)?)?)?)@', '<a target="_blank" href="$1">$1</a>', $string);

       // Convert usernames (@) into links 
       $string = preg_replace("(@([a-zA-Z0-9\_]+))", 
            "<a target='_blank' href=\"https://pinterest.com/\\1\">\\0</a>", $string);

       // Convert hash tags (#) to links 
       $string = preg_replace('/(^|\s)#(\w+)/', 
            '\1<a target="_blank" href="https://pinterest.com/search/?q=\2">#\2</a>', $string);
